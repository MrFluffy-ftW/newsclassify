...
class Scrape_Nepali_News:
    ...
    def get_ekantipur_scraped_news(self,scrape_link):
        ...
        for link in scrape_link:
            ...
            if confidence > 30:
                self.scraped_dict[count] = {'link':link,'title':title,'news':news_str,'source':source,'category':category,'date':date,'confidence':confidence}
            else:
                self.scraped_dict[count] = {'link':link,'title':title,'news':news_str,'source':source,'category':'OTHERS','date':date,'confidence':confidence}
            count += 1
        
    def create_table(self):
        table = " CREATE TABLE IF NOT EXISTS nepali_news(pk_categoryid INTEGER PRIMARY KEY AUTOINCREMENT,title VARCHAR(100), news VARCHAR(2000), source VARCHAR(100), link VARCHAR(100),category VARCHAR(15),date VARCHAR(25),confidence BIGINT)"
        self.cursor_obj.execute(table)
    
    def update_table(self,key):
        try:
            self.cursor_obj.execute("INSERT INTO nepali_news (title,news,source,link,category,date,confidence) VALUES (?,?,?,?,?,?,?)",(self.scraped_dict[key]['title'],self.scraped_dict[key]['news'],self.scraped_dict[key]['source'],self.scraped_dict[key]['link'],self.scraped_dict[key]['category'],self.scraped_dict[key]['date'],self.scraped_dict[key]['confidence']) )
            self.con.commit()
