...
def fetch_scrapped_english_news():
    ...
    if request.method != 'POST': 
        ...
        cur = con.cursor()
        con.row_factory = sql.Row
        cur.execute("SELECT * FROM english_news WHERE category =? ORDER BY confidence DESC", ["BUSINESS"])
        fetch_row = cur.fetchall()
        if len(fetch_row) != 0:
            index = 0
            next_item_index = 0 
        ...
        if request.json["changeCategory"] != "":
            ...
            cur.execute("SELECT * FROM english_news WHERE category = ? ORDER BY confidence DESC", [change_category])
            fetch_row = cur.fetchall()
            if len(fetch_row) != 0:
                index = 0
                next_item_index = 0 
